...
function loadCodexManifest(): PlatformManifest {
  return {
    commands: [],
    agents: [],
    skills: [],
...
function listSkillsInDir(skillsDir: string, prefix: string): Resource[] {
...
function listSkillsRes(): Resource[] {
...
function listRulesRes(): Resource[] {
  const paths = getCodexPaths();
...
export function createCodexBackend(): import("./types.js").PlatformBackend {
  return {
    platform: "codex",
    hasPaths() {
      const paths = getCodexPaths();
