# Config MCP Server (Cursor, Claude Code, Codex)

MCP server that exposes **commands**, **skills**, **agents**, **rules**, and **hooks** from **Cursor**, **Claude Code**, and **Codex** as MCP resources and tools. One server works across all three environments so any MCP client can discover and read config from any supported platform.
...
| `list_agents` | List all agents (Cursor, Claude, Codex e.g. AGENTS.md). |
| `list_skills` | List all skills across platforms. |
| `list_rules` | List all rules across platforms. |
| `list_hooks` | List hooks (Cursor only). |
| `get_command` | Get a command by id. Optional `platform`: `cursor`, `claude`, `codex`. |
...
| `get_hook` | Get a hook by id. Optional `platform` (Cursor only). |
